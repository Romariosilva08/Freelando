...
public static class PropostasExtensions
{
    public static void AddEndPointPropostas(this WebApplication app)
    {
        ...
        {
            var propostas = await unitOfWork.contexto.Propostas.FromSqlRaw("EXEC dbo.sp_BuscarTodasPropostas").ToListAsync();
        ...
        {
            //PropostaSummary não existe como tabela e nem como DBSet no nosso projeto. Por isso, a partir do contexto, executamos o Database e executamos o comando SqlQueryRaw().
            var propostasSummary = await unitOfOrk.contexto.Database.SqlQueryRaw<PropostaSummary>("EXEC dbo.sp_PropostaSummary").ToListAsync();
            return Results.Ok(propostasSummary);
        }).WithTags("Propostas").WithOpenApi();
